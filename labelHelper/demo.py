...
#             img = model(image_path,save_dir = output_folder)  # 对图像进行预测
...
class YOLOv8Seg:
    ...
    def __init__(self, onnx_model: str, conf: float = 0.25, iou: float = 0.7, imgsz: Union[int, Tuple[int, int]] = 640,classes_file=''):
        """
        ...
        self.classes = YAML.load(check_yaml(classes_file))["names"]
...
if __name__ == "__main__":
    ...
    model = YOLOv8Seg(modelPath, conf, iou,imgsz = 640*2,classes_file = '/home/ndvision/dl/ultralytics/train/segment/chip/dataset.yaml')
    os.makedirs(output_folder, exist_ok=True)
    for image_file in os.listdir(image_folder):
        if image_file.endswith(".jpg") or image_file.endswith(".png") or image_file.endswith(".bmp"):
            image_path = os.path.join(image_folder, image_file)
            ...
            # img = cv2.resize(img,(640*2,640*2))
            results = model(img)
            # 保存可视化图像到文件夹
            if len(results)==0:
                cv2.imwrite(output_filename, img)
